...
The search script lives inside this skill's own directory, not the project directory. The paths below are relative to the repository root — run them from there, or prefix them with the repository root if the working directory is elsewhere:
